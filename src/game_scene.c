...
PGB_GameScene *PGB_GameScene_new(const char *rom_filename)
{
    ...
    if (rom)
    {
        ...
        enum gb_init_error_e gb_ret = gb_init(
            &context->gb, context->wram, context->vram, rom, gb_error, context);
...
__attribute__((optimize("Ofast"))) static void PGB_GameScene_update(
    void *object)
{
    ...
    gameScene->selector.toggleY =
        roundf((gameScene->selector.height - gameScene->selector.toggleSize) *
               progress);
    ...
    if (needsDisplay ||
        ...
        gameScene->model.selectorStartPressed !=
            gameScene->selector.startPressed ||
        gameScene->model.selectorSelectPressed !=
    ...
    if (gameScene->state == PGB_GameSceneStateLoaded)
    {
        ...
        bool gb_draw = (!context->gb.direct.frame_skip ||
            ...
            for (int y = 0; y < (LCD_HEIGHT - 1); y++)
            {
                ...
                if (memcmp(pixels, old_pixels, LCD_WIDTH) != 0)
                {
                    ...
                    for (int x = 0; x < LCD_WIDTH; x++)
                    {
                        ...
                        framebuffer[fb_index1] |=
                            PGB_bitmask[pixel][bit][d_row1];
                        if (y_offset == 2)
                        {
                            framebuffer[fb_index2] |=
                                PGB_bitmask[pixel][bit][d_row2];
                        }
        ...
        unsigned int rtc_delta =
            playdate->system->getSecondsSinceEpoch(NULL) - gameScene->rtc_time;
        ...
        if (needsDisplaySelector)
        {
            ...
            playdate->graphics->drawBitmap(PGB_App->selectorBackground,
                                           selector->x, selector->y,
                                           kBitmapUnflipped);

            if (selector->startPressed && selector->selectPressed)
            {
                playdate->graphics->drawBitmap(PGB_App->selectorFilledButton,
                                               selector->x, selector->y,
                                               kBitmapUnflipped);
                playdate->graphics->drawBitmap(
                    PGB_App->selectorFilledButton, selector->x,
                    selector->y + gameScene->selector.height -
                        selector->toggleSize,
                    kBitmapUnflipped);
            }
            else
            {
                ...
                playdate->graphics->drawBitmap(toggleImage, selector->x,
                                               selector->y + selector->toggleY,
                                               kBitmapUnflipped);
            }
        }
